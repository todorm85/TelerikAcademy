    ...
    public class GamesController : BaseApiController
    {
        ...
        public IHttpActionResult Join(Guid id)
        {
            ...
            var game = this.data.Games
                .All()
                .Where(g => g.Id == id && g.State == GameState.WaitingForSecondPlayer && g.FirstPlayerId != currentUserId)
                .FirstOrDefault();
        ...
        public IHttpActionResult List()
        {
            ...
            var availableGames = this.data.Games.All().Where(x => x.SecondPlayerId == null && x.FirstPlayer.Id != userId)
                .Select(x => new {
                    Id = x.Id,
                    FirstPlayer = x.FirstPlayer.UserName,
                    SecondPlayer = x.SecondPlayer.UserName,
                    Status = x.State
                });
        ...
        [HttpGet]
        public IHttpActionResult ListMy()
        {
            var userId = this.userIdProvider.GetUserId();
            var myGames = this.data.Games.All().Where(x => x.FirstPlayer.Id == userId || x.SecondPlayerId == userId)
                .Select(x => new {
                    Id = x.Id,
                    FirstPlayer = x.FirstPlayer.UserName,
                    SecondPlayer = x.SecondPlayer.UserName,
        ...
        {
            var userId = this.userIdProvider.GetUserId();
            var game = this.data.Games.All().FirstOrDefault(x => x.FirstPlayerId == userId && x.Id == id);
